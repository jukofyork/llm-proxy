...
public class LlmProxy extends HttpProxy {
    ...
    private static class ModelRequestRouter implements RequestRouter {
        ...
        public ProxyTarget route(String method, String requestPath, String requestBody, Map<String, String> headers) {
            ...
            return new ProxyTarget(targetUri, targetModelConfig.apiKey(), isStreaming, transformedBody);
        }
        ...
        private String transformRequestBody(String requestBody, String modelName) {
            ...
            try {
                ...
                // Apply disallowed parameter filtering first (simple top-level only)
                if (serverConfig.disallowedParams() != null) {
                    for (String param : serverConfig.disallowedParams()) {
                        if (!param.contains(".") && objectNode.has(param)) {
                            objectNode.remove(param);
                        }
                    }
                }
                
                // Apply parameter overrides
                if (serverConfig.params() != null) {
